 ...
 * 
 * CHANGELOG v3.03:
 * - Fixed React Hooks rules violations: moved useState/useEffect calls before early returns
...
export default function PokemonCareerGame() {
  ...
  if (gameState === 'tournamentReplay' && selectedReplay) {
    ...
    // Reconstruct battle state from server's battle log
    ...
    }, [replayTick, isPlaying, replaySpeed, maxTicks]);
    
    // Get current battle state at this tick
    const getCurrentState = () => {
      if (replayTick >= battleLog.length) return battleLog[battleLog.length - 1];
      return battleLog[replayTick];
    };
    
    const currentState = getCurrentState();
    const player = currentState?.player || { name: 'Player 1', currentHP: 0, maxHP: 100, currentStamina: 0, maxStamina: 100 };
    const opponent = currentState?.opponent || { name: 'Player 2', currentHP: 0, maxHP: 100, currentStamina: 0, maxStamina: 100 };
    const log = currentState?.log || [];
      ...
      <div className="w-full h-screen bg-gradient-to-b from-gray-700 to-gray-900 flex items-center justify-center p-2 sm:p-4">
        <div className="w-full max-w-4xl">
          ...
          <div className="bg-white rounded-lg p-4 mb-4 shadow-lg">
            <div className="flex items-center justify-between flex-col sm:flex-row gap-3">
              <div>
                <h2 className="text-xl font-bold text-purple-600">Tournament Battle Replay</h2>
                <p className="text-sm text-gray-600">
                  {selectedReplay.player1_username} vs {selectedReplay.player2_username}
                </p>
              </div>
              <button
                onClick={() => {
          ...
          {/* Battle Arena */}
          <div className="bg-gradient-to-b from-green-300 to-green-500 rounded-lg p-4 sm:p-6 shadow-2xl mb-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
              {/* Player Side */}
              <div className="bg-white bg-opacity-90 rounded-lg p-3 sm:p-4">
                <div className="flex items-center gap-2 mb-2">
                ...
                <div className="mb-2">
                  <div className="flex justify-between text-xs mb-1">
                    <span>HP</span>
                    <span className="font-bold">{Math.max(0, player.currentHP)}/{player.maxHP}</span>
                  </div>
                ...
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span>Stamina</span>
                    <span className="font-bold">{Math.max(0, player.currentStamina)}/{player.maxStamina}</span>
                  </div>
                  <div className="w-full bg-gray-300 rounded-full h-2">
                    <div
                      className="bg-yellow-400 h-2 rounded-full transition-all duration-300"
                      style={{ width: `${Math.max(0, (player.currentStamina / player.maxStamina) * 100)}%` }}
                    ></div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white bg-opacity-90 rounded-lg p-3 sm:p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h3 className="font-bold text-sm sm:text-base">{opponent.name}</h3>
                </div>
                <div className="mb-2">
                  <div className="flex justify-between text-xs mb-1">
                    <span>HP</span>
                    <span className="font-bold">{Math.max(0, opponent.currentHP)}/{opponent.maxHP}</span>
                  </div>
                  <div className="w-full bg-gray-300 rounded-full h-3">
                    <div
                      className="bg-green-500 h-3 rounded-full transition-all duration-300"
                ...
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span>Stamina</span>
                    <span className="font-bold">{Math.max(0, opponent.currentStamina)}/{opponent.maxStamina}</span>
                  </div>
                  <div className="w-full bg-gray-300 rounded-full h-2">
                    <div
                      className="bg-yellow-400 h-2 rounded-full transition-all duration-300"
                      style={{ width: `${Math.max(0, (opponent.currentStamina / opponent.maxStamina) * 100)}%` }}
                    ></div>
                  </div>
                </div>
              </div>
            ...
            {/* Battle Log */}
            <div className="bg-black bg-opacity-80 rounded-lg p-3 max-h-32 overflow-y-auto">
            ...
            <div className="flex items-center justify-between gap-4 flex-wrap">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-4 py-2 bg-purple-600 text-white rounded font-bold hover:bg-purple-700 transition text-sm"
                >
